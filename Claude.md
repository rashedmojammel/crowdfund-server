...
- Withdrawal approval (`approveWithdrawal`, admin-only): flips status to "approved" and notifies the creator — never touches `User.credits` or any wallet field, since nothing else needs to move. Idempotent: an already-approved withdrawal is a no-op that does not re-notify. One transaction.
- Payments: `credits`/`amountUsd` must exactly match one of the four fixed `CREDIT_PACKAGES` — arbitrary amounts are rejected before Stripe is ever called. Checkout session metadata carries `{ supporterEmail: fromJWT, credits: fromValidatedPackage }`, never a client-suppliable amount. The wallet is credited by the webhook route (idempotent via `stripeSessionId`), not by this route.
- Reports (`handleReportAction` in `lib/reports.ts`, admin-only): resolve/dismiss just update report status; suspend_campaign also suspends the reported campaign (via `setCampaignStatus`); delete_campaign also deletes it with refunds (via `deleteCampaignWithRefunds`, shared with `DELETE /api/campaigns/[id]` — do not duplicate that logic). Idempotent: a report already in the target status is a no-op; a report closed with a different outcome is a 409. reporterEmail from JWT.
